...
class S4MidpointOffsetCoder(BaseBBoxCoder):
    ...
    def decode(self,
        ...
        decoded_bboxes = delta_sp2bbox(bboxes, pred_bboxes, self.means, self.stds, wh_ratio_clip)
...
def bbox2delta_sp(proposals, gt,
    ...
    px, py, pw, ph, _ = (proposals.float()).unbind(dim=-1)
    gx, gy, gw, gh, gtheta = (gt.float()).unbind(dim=-1)
    #gtheta = gtheta0-ptheta
    gsin = gtheta.sin()

    gw_regular = torch.where(gsin**2<=0.5, gw, gh)
    gh_regular = torch.where(gsin**2<=0.5, gh, gw) 
    dtheta_regular = torch.where(gsin**2<=0.5, gtheta, gtheta-pi/2*gsin.sign())
...
def delta_sp2bbox(rois, deltas,
    ...
    px, py, pw, ph, _ = rois.unbind(dim=-1)
    
    px = px.unsqueeze(1).expand_as(dx)
    py = py.unsqueeze(1).expand_as(dx)
    pw = pw.unsqueeze(1).expand_as(dx)
    ph = ph.unsqueeze(1).expand_as(dx)
    ...
    thetap1 = (dthetap+pi/4)%(pi/2)-(pi/4)#change
