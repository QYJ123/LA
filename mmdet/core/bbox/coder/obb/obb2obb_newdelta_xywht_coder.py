...
class OBB2OBBDeltaXYWHT2Coder(BaseBBoxCoder):
    ...
    def encode(self, bboxes, gt_bboxes):
        ...
        encoded_bboxes = obb2delta(bboxes, gt_bboxes, self.means, self.stds,self.lamda)
    ...
    def decode(self,
        ...
        decoded_bboxes = delta2obb(bboxes, pred_bboxes, self.means, self.stds,
                                   wh_ratio_clip,self.lamda)

        return decoded_bboxes


def obb2delta(proposals, gt, means=(0., 0., 0., 0., 0.), stds=(1., 1., 1., 1., 1.),lamda_r=1):
...
def delta2obb(proposals,
              ...
              means=(0., 0., 0., 0., 0.),
              stds=(1., 1., 1., 1., 1.),
              wh_ratio_clip=16/1000,lamda_r=1):
    ...
    gh = ph * dh.exp()
    gr = gw/gh*(gw>=gh)+gh/gw*(gw<gh)
    #gtheta = dtheta/(1+(lamda_r*(1-gr)).exp()) + ptheta
    gtheta = dtheta/(lamda_r*gr) + ptheta
